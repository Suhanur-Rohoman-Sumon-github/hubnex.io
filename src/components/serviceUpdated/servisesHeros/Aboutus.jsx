...
const Aboutus = () => {
    ...
    const getTestimonials = async () => {
      try {
        const res = await fetch(
          `https://${import.meta.env.VITE_API_URL}/api/v1/testimonials`
        );
  ...
  return (
    <>
      <div
        className="w-full flex gap-5 md:gap-24 justify-center items-center h-[617px] flex-col xl:w-full "
        id="display"
      >
              ...
              <SwiperSlide className=" py-12">
                <div className=" flex flex-col gap-5 md:gap-8 justify-center items-center w-full">
                  ...
                  <div className=" w-fulls items-center flex flex-col justify-between">
                    <h1 className="font-gilroy-bold text-[26px]">
                      {data.author}
                    </h1>
                    <h2>{data.role}</h2>
                  </div>
